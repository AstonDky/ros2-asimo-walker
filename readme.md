...
| `step_time` | `1.62` | `common.py: WalkerParams.step_time` | 单脚摆动持续时间。增大更慢更稳；减小动作更快但更容易甩动和跌倒。 |
| `double_support_time` | `0.48` | `common.py: WalkerParams.double_support_time` | 双脚支撑落脚后的稳定时间。增大有利于站稳和恢复，减小会让步态更连续但风险更高。 |
| `foot_clearance` | `0.052` | `common.py: WalkerParams.foot_clearance` | 摆动脚抬脚高度。增大能减少拖地，但会提高单脚支撑扰动；减小更稳但可能扫地。 |
...
| `zmp_kd` | `1.0` | `common.py: WalkerParams.zmp_kd` | CoM 速度阻尼。增大可抑制速度和超调，过大会走不动；减小响应更快但更容易晃。 |
...
| `transfer_time` | `0.74` | `common.py: WalkerParams.transfer_time` | 双脚支撑时重心转移到支撑脚的时间。增大更稳，减小更快但容易在抬脚前重心不到位。 |
...
| `swing_lift_fraction` | `0.24` | `common.py: WalkerParams.swing_lift_fraction` | 摆腿周期中抬脚阶段占比。增大抬脚更慢，减小更快达到高点。 |
| `swing_lower_fraction` | `0.26` | `common.py: WalkerParams.swing_lower_fraction` | 摆腿周期中落脚阶段占比。增大落脚更慢，减小会更快下落。 |
| `dt` | `0.02` | `common.py: WalkerParams.dt` | 控制循环周期，约 50 Hz。改动会影响定时器、限速和所有时序计算。 |
| `max_joint_rate` | `1.55` | `common.py: WalkerParams.max_joint_rate` | 腿部关节目标角速度上限。增大动作更快但冲击更大；减小更柔和但可能跟不上步态。 |
...
| `step_width` | `0.09` |
| `step_time` | `1.62` |
| `double_support_time` | `0.48` |
| `foot_clearance` | `0.052` |
| `pelvis_height` | `0.48` |
| `total_steps` | `6` |
| `sagittal_sign` | `-1.0` |
...
| `touchdown_time` | `0.25` |
| `stand_time` | `2.0` |
| `swing_lift_fraction` | `0.24` |
| `swing_lower_fraction` | `0.26` |
| `dt` | `0.02` |
| `max_joint_rate` | `1.55` |
| `max_arm_rate` | `1.2` |
| `max_com_speed` | `0.075` |
