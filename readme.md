# CoppeliaSim Asti ROS 2 Walking Control Experiment

## 项目意图

这个项目的目标是驱动 CoppeliaSim 内置的 Asti 双足机器人进行从零开始的运动控制与行走控制实验。Asti 的外形和控制目标都很接近本田 ASIMO 这类传统双足机器人，目前目的是逐步实现一个更接近 ASIMO 风格的传统行走控制栈。

最终希望达到的效果是：在 ROS 2 命令或遥操作节点里发布键盘控制指令，通过 `W/A/S/D/Q/E` 等按键让仿真环境中的机器人平滑完成前进、左转朝向、后退、右转朝向、左扭动腰部、右扭动腰部等操作。当前代码已经实现保守的 ZMP/CoM 双足步行动作，后续遥操作层应该在这个稳定步态基础上扩展，考虑加入RL。
...
后续如果加入键盘遥操作、腰部扭转、转向步态、强化学习、MPC、全身控制或其他新模块，需要同步更新本 README 的项目意图、代码结构、控制方法、可调参数和构建历史。
